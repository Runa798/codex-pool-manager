...
def _load_config():
    ...
    config = {
        ...
        "tempmail_admin_auth": "",             # temp-mail 的 admin 密码
        "tempmail_domain": "domain.com",         # 创建邮箱时使用的域名
        "proxy": "",
    ...
    config["tempmail_admin_auth"] = os.environ.get("TEMPMAIL_ADMIN_AUTH", config["tempmail_admin_auth"])
...
TEMPMAIL_ADMIN_AUTH = _CONFIG["tempmail_admin_auth"]
...
if MAIL_PROVIDER == "tempmail":
    if not TEMPMAIL_ADMIN_AUTH:
        ...
        print("   环境变量: export TEMPMAIL_ADMIN_AUTH='your_admin_password'")
...
def _extract_verification_code(email_content: str):
    ...
    patterns = [
        r"Your ChatGPT code is\s+(\d{6})",   # temp-mail raw 邮件 Subject/正文
...
class ChatGPTRegister:
    ...
    def _create_tempmail_session(self):
        """创建 temp-mail 请求会话"""
        session = curl_requests.Session()
        session.headers.update({
            "User-Agent": self.ua,
    ...
    def create_temp_email_tempmail(self):
        """生成随机邮箱地址（无需调用 API 创建），返回 (email, password, mail_token)。
        mail_token 直接复用 email 地址字符串，读取邮件时通过 admin API 按 address 过滤。
        """
        if not TEMPMAIL_ADMIN_AUTH:
            raise Exception("TEMPMAIL_ADMIN_AUTH 未设置，无法使用 temp-mail")
    ...
    def _fetch_emails_tempmail(self, mail_token: str):
        ...
        try:
            session = self._create_tempmail_session()
            headers = {"x-admin-auth": TEMPMAIL_ADMIN_AUTH}
            res = session.get(
                f"{TEMPMAIL_API_BASE}/admin/mails",
                params={"limit": "10", "offset": "0", "address": mail_token},
                headers=headers,
                timeout=15,
            ...
            if res.status_code == 200:
                ...
                return (data.get("results") or data.get("items")
                        or data.get("data") or data.get("mails") or [])
        except Exception:
            pass
        return []

    def _fetch_email_detail_tempmail(self, mail_token: str, msg_id):
        """获取 temp-mail 单封邮件详情（列表条目已包含 text/html，直接返回）"""
        # temp-mail /api/mails 列表条目通常已含邮件正文，无需单独获取详情
        return None
    ...
    def _wait_for_verification_email_tempmail(self, mail_token: str, timeout: int = 120):
        """等待并提取 OpenAI 验证码 (temp-mail)
        temp-mail admin/mails 返回的每条邮件含 raw 字段（完整 MIME 报文），
        Subject 行格式为 'Your ChatGPT code is 123456'。
        """
        self._print(f"[OTP] 等待验证码邮件 (temp-mail, 最多 {timeout}s)...")
        start_time = time.time()

        while time.time() - start_time < timeout:
            messages = self._fetch_emails_tempmail(mail_token)
            if messages:
    ...
    def visit_homepage(self):
        ...
        if r.status_code == 403:
            raise Exception(f"visit_homepage 返回 403，IP 可能被封锁或缺少代理，请更换代理后重试")

    def get_csrf(self) -> str:
        url = f"{self.BASE}/api/auth/csrf"
        r = self.session.get(url, headers={"Accept": "application/json", "Referer": f"{self.BASE}/"})
        if r.status_code != 200:
            raise Exception(f"get_csrf 请求失败: HTTP {r.status_code} — {r.text[:200]}")
        try:
            data = r.json()
        except Exception as e:
            raise Exception(f"get_csrf 响应不是有效 JSON (status={r.status_code}): {r.text[:200]}")
...
def main():
    ...
    if MAIL_PROVIDER == "tempmail":
        if not TEMPMAIL_ADMIN_AUTH:
            ...
            print("   Windows: set TEMPMAIL_ADMIN_AUTH=your_admin_password")
            print("   Linux/Mac: export TEMPMAIL_ADMIN_AUTH='your_admin_password'")
